...
class MySqlDB extends \fw\database\DataBase
{
    ...
    public function connect($host, $user, $password, $dbname) {
        if ($this->trace) {echo gtab(1)."Enter ".__METHOD__.$host."/".$user."/".$password."/".$dbname."<br>\n"; }
        try {
            ...
            if ($this->mysqli->connect_errno) {
                ...
                $this->mysqli->query("SET SESSION group_concat_max_len = 65536;");
                if (!$this->mysqli->autocommit(true) ) { 
                    echo gtab(). $this->mysqli->error."<br>\n";
                }
            }
    ...
    public function processresults($query_result,&$results,$logresult,$procedure){
    ...
    public function executefunction($function,$parameters,&$results, &$numrows=0, $trace=false,$log=0,$logquery=0,$logresult=0){
        if ($this->trace || $trace ) { echo gtab(1)."Enter : ",__METHOD__.":".$function,"<br>\n\n"; }
        $query = '';
        ...
        if ($success) {
            $success = $this->processresults($query_result,$results,$logresult,$function);
            // $this->buildresultsarray($query_result,$results);
            // if ($logresult)  {
            //     $this->errorhandler->dblog($procedure." RESULT>>\n ".print_r($results,1));
            // }
            ...
            if ($logresult)  {
                $this->errorhandler->dblog($function." FAILED\n ".print_r($results,1));
            }
            $results[] = [];
        }
        $results = array_values($results[0]);
        if ($this->trace || $trace) { echo gtab(-1)."Leave ".__METHOD__.":".$function."(".$numrows." rows returned)<br>\n";var_dump($results); }
        return $success;
     }
    public function executeprocedure($procedure,$parameters,&$resultset,&$outparams,&$numrows=0, $trace=false,$logquery=0,$logresult=0) {
        ...
        if ($this->trace || $trace) { echo gtab(1)."Enter ".__METHOD__.":".$procedure."<br>\n"; }
        global $ob;
        $success = true;
        $query = '';
        ...
        try {
            if ($this->mysqli->begin_transaction()) {
                ...
                if ($result == false) {
                    ...
                    if ( $this->trace || $trace) { echo gtab().$errormessage; }
                    $success = false;
                    $this->mysqli->rollback();
                } else {
                    if ($querytype == 0 || substr($query,0,6)=="SELECT") { //SELECT, SHOW, DESCRIBE or EXPLAIN -these return a table of results
                        $numrows = $result->num_rows;
                        if ( $this->trace || $trace) {echo gtab()."Number of rows in result = ".$numrows."<br>\n"; }
                    } else if ($querytype == 1) { //CREATE, INSERT, UPDATE, DELETE, ETC
                        $this->insert_id = $this->mysqli->insert_id;
                        if (substr($query,0,6)=="INSERT") {
                            $result = $this->insert_id;
                        }
                        if ( $this->trace || $trace) {echo gtab(). "Number of rows affected = ".$numrows."<br>\n"; }
                    } else  {// SET, etc
                        if ($this->trace  || $trace) {echo gtab()."Command completed<br>\n".$query."<br>\n============<br>\n"; }
                    }
                    $success = true;
                    $this->mysqli->commit();
                }
                ...
                if ( $this->trace || $trace) {echo $this->mysqli." Failed starting transaction.<br>\n"; }
                $success = false;
             }
        } catch(\Exception|\mysqli_sql_exception $e) {
            $errormessage .= "Query Failed::(".$e->getCode().")<br>\n*<br>\n".$e->getMessage()."<br>\n<br>\n".$query;
            if (!$noerrorhandler) {
                $emailmessage = $e->getCode()." ".$e->getMessage()."\n\n".$e->__toString();
                $this->errorhandler->sqlerror($this->mysqli,"$emailmessage\n\n",$query);
            }
            if ( $this->trace || $trace) {echo $errormessage."<br>\n"; }
            $success = false;        
        }
